...
try:
    ...
    # Add project root (4 levels up from services/s3/downloader.py)
...
REGION = 'us-east-1'
OFFER_INDEX_URL = 'https://pricing.us-east-1.amazonaws.com/offers/v1.0/aws/AmazonS3/current/region_index.json'
...
def download():
    ...
    try:
        ...
        if not region_url_suffix:
            logger.error(f"S3: Could not find URL for region {REGION}")
            return
        ...
        output_file = Path(output_dir) / 's3.json'
        
        logger.info(f"S3: Downloading pricing from {price_url} to {output_file}")
