...
def populate_temp_terms(conn: sqlite3.Connection, raw_file_path: str):
    logger.info("EFS: Pass 1 - Extracting pricing terms...")
    ...
    try:
        ...
        logger.error(f"EFS: Term extraction failed: {e}")
        raise
    logger.info(f"EFS: Pass 1 complete. Loaded {count} terms.")
...
def normalize(raw_file, output_db_path=None):
    ...
    logger.info(f"EFS: Checking normalization for {raw_file}...")
    
    raw_path = Path(raw_file)
    if not raw_path.exists():
        logger.error(f"EFS: Raw file {raw_file} not found.")
        return

    raw_mtime = raw_path.stat().st_mtime
    
    conn = get_db_connection(output_db_path)
    create_pricing_table(conn, 'efs')
    create_index(conn, 'efs', ['storageClass', 'usagetype', 'location'])
    
    if should_skip_normalization(conn, 'efs', raw_mtime):
        logger.info("EFS: Data is up-to-date. Skipping normalization.")
        conn.close()
        return
    ...
    try:
        ...
        # 2. Process Products
        logger.info("EFS: Pass 2 - Normalizing products...")
        ...
        with open(raw_file, 'rb') as f:
            for sku, product in ijson.kvitems(f, 'products'):
                ...
                if len(batch) >= batch_size:
                    insert_pricing_batch(conn, 'efs', batch)
                    count += len(batch)
                    batch = []
        ...
        update_normalization_metadata(conn, 'efs', raw_mtime)
        logger.info(f"EFS: Normalization complete. inserted {count} rows.")
        
        summary_path = raw_path.parent.parent / 'normalized' / 'efs' / 'summary.json'
