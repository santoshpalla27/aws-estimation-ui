...
def populate_temp_terms(conn: sqlite3.Connection, raw_file_path: str):
    logger.info("RDS: Pass 1 - Extracting pricing terms...")
    ...
    try:
        ...
        logger.error(f"RDS: Term extraction failed: {e}")
        raise
    logger.info(f"RDS: Pass 1 complete. Loaded {count} terms.")
...
def normalize(raw_file, output_db_path=None):
    ...
    logger.info(f"RDS: Checking normalization for {raw_file}...")
    
    raw_path = Path(raw_file)
    if not raw_path.exists():
        logger.error(f"RDS: Raw file {raw_file} not found.")
        return

    raw_mtime = raw_path.stat().st_mtime
    
    conn = get_db_connection(output_db_path)
    create_pricing_table(conn, 'rds')
    create_index(conn, 'rds', ['instanceType', 'databaseEngine', 'deploymentOption', 'databaseEdition', 'location', 'storage'])
    
    if should_skip_normalization(conn, 'rds', raw_mtime):
        logger.info("RDS: Data is up-to-date. Skipping normalization.")
        conn.close()
        return
    ...
    try:
        ...
        # 2. Process Products
        logger.info("RDS: Pass 2 - Normalizing products...")
            ...
            for sku, product in ijson.kvitems(f, 'products'):
                ...
                item = {
                    ...
                    "attributes": {
                        ...
                        "licenseModel": attr.get('licenseModel'),
                        "vcpu": attr.get('vcpu'),
                ...
                if len(batch) >= batch_size:
                    insert_pricing_batch(conn, 'rds', batch)
                    count += len(batch)
                    batch = []
        ...
        update_normalization_metadata(conn, 'rds', raw_mtime)
        logger.info(f"RDS: Normalization complete. inserted {count} rows.")
        
        summary_path = raw_path.parent.parent / 'normalized' / 'rds' / 'summary.json'
